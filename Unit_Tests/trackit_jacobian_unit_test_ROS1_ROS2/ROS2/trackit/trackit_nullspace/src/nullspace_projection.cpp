 ...
 */
  ...
  * @throws None
  */
 NullSpace::NullSpace() : Node("NullSpace") {
	 // private nodehandle
	 // _nh = ros::NodeHandle("~");
 
	 // node parameters
	 _nh->declare_parameter("lambda", 0.05);
	 _nh->get_parameter("lambda", _lambda);
	 _nh->declare_parameter("time_diff_threshold", 0.01);
	 _nh->get_parameter("time_diff_threshold", _time_diff_threshold);
 
	 // subscribers
	 // _sub_joint_vel = _n.subscribe("joint_velocity_in", 1, &NullSpace::jointVelocityCallback, this);
	 _sub_joint_vel = this->create_subscription<trackit_msgs::msg::JointVelocityStamped>(
		 "joint_velocity_in", 1, std::bind(&NullSpace::jointVelocityCallback,this,std::placeholders::_1));
	 // _sub_jacobian  = _n.subscribe("jacobian_in", 1, &NullSpace::jacobianCallback, this);
	 _sub_jacobian = this->create_subscription<trackit_msgs::msg::JacobianStamped>(
		 "jacobian_in", 1, std::bind(&NullSpace::jacobianCallback,this,std::placeholders::_1));
 
	 // publishers
	 // _pub_projected_joint_vel = _n.advertise<trackit_msgs::JointVelocityStamped>("joint_velocity_out", 1);
	 _pub_projected_joint_vel = this->create_publisher<trackit_msgs::msg::JointVelocityStamped>("joint_velocity_out", 1);
  ...
  * @throws None
  */
 bool NullSpace::isMsgCurrent(rclcpp::Time time) {
	 // check if the current time and the given time is within the time difference threshold
	 rclcpp::Time now = this->get_clock()->now();
	 if ((now.nanoseconds()/1000000 - time.nanoseconds()/1000000) > _time_diff_threshold) {
		 return false;
	 }
	 else {
		 return true;
  ...
  * @throws None
  */
 void NullSpace::jointVelocityCallback(const trackit_msgs::msg::JointVelocityStamped::SharedPtr msg) {
  ...
  * @throws None
  */
 void NullSpace::jacobianCallback(const trackit_msgs::msg::JacobianStamped::SharedPtr msg) {
	 trackit_msgs::msg::JacobianStamped jacobian_msg = *msg;
	 ...
	 if(isMsgCurrent(_msg_joint_velocity.header.stamp)) {
		 ...
		 trackit_msgs::msg::JointVelocityStamped eig_projected_joint_velocity_msg;
  ...
  * @throws None
  */
 Eigen::MatrixXd NullSpace::jointVelocityMsgToEigen(const trackit_msgs::msg::JointVelocityStamped msg) {
  ...
  * @throws None
  */
 std_msgs::msg::Float64MultiArray NullSpace::eigenToJointVelocityMsg(const Eigen::MatrixXd& eig_joint_velocity) {
 ...
 int main(int argc, char **argv) {
	 ...
	 auto node = std::make_shared<NullSpace>();
